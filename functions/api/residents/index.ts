export async function onRequestGet({ env, request }: { env: { DB: D1Database }; request: Request }) {
  try {
    ...
    if (houseTypes.length) {
      where.push(`(COALESCE(houseType, house_type) IN (${houseTypes.map(() => '?').join(', ')}))`)
      params.push(...houseTypes)
    }

    if (filter) {
      where.push('(COALESCE(houseNo, house_no) LIKE ?)')
      params.push(`%${filter}%`)
    }
...
export async function onRequestPost({ env, request }: { env: { DB: D1Database }; request: Request }) {
  try {
    ...
    if (!tableCheck) {
      ...
      await env.DB.prepare(
        `CREATE UNIQUE INDEX IF NOT EXISTS idx_residents_house_no ON residents(house_no)`
      ).run()
    }
    ...
    const existingHouse = await env.DB.prepare(
      'SELECT id FROM residents WHERE house_no = ?'
    ).bind(houseNo).first()
    ...
    return new Response(JSON.stringify({ error: 'Failed to create resident' }), {
      status: 500,
      headers: { 'content-type': 'application/json' },
    })
  }
