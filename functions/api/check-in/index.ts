...
export async function onRequestGet({ request, env }: { request: Request, env: Env }) {
  try {
    ...
    if (!userId || !checkpointId) {
      return new Response(JSON.stringify({ error: 'Missing userId or checkpointId' }), { status: 400 })
    }
    ...
    `).bind(userId, checkpointId).first()
    ...
    return new Response(JSON.stringify({ error: 'Failed to fetch last check-in' }), { status: 500 })
  }
}
...
export async function onRequestPost({ request, env }: { request: Request, env: Env }) {
  try {
    ...
    const lastCheckIn = await env.DB.prepare(`
      ...
      ORDER BY timestamp DESC LIMIT 1
    `).bind(userId, nearestCheckpoint.id, cutoffTime).first()
    ...
    await env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).bind(id, userId, nearestCheckpoint.id, latitude, longitude, timestamp).run()
